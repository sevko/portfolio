"""
Contains functions for compiling the `syntax.parser.ParseTreeNodes` generated
by the grammar rules in `syntax.parser`.
"""
...
def compile_(node, vm_writer, sym_tables):
	"""
	Either compiles a node with a `_compile_` function specific to its type, or
	simply recursively `compile_()`s all of its children.

	Args:
		node (`syntax.parser.ParseTreeNode`): A node in the parse tree
			generated by `syntax.parser` nodes.
		vm_writer (`vm_writer.VMWriter`): The VMWriter to write the Virtual
			Machine code for this node to.
		sym_tables (dict of `symbol_table.SymbolTable`): A dictionary
			containing two keys, `"local"` and `"global"`, each of which maps
			to a `SymbolTable` used to keep track of symbols in that scope.
	"""
