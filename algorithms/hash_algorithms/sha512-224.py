"""
`sha512-244` contains an implementation of the SHA-512/244 hash algorithm.
...
def sha512_224(string):
	"""
	Generate the SHA-512-224 digest of a string.
	...
	constants = [h0, h1, h2, h3]
	digest = 0
	for ind in xrange(len(constants)):
		digest += constants[ind] << (64 * (len(constants) - ind - 1))
	return digest >> (4 * 64 - 224) # Cap digest at 224 bits.

if __name__ == "__main__":
	msg = ""
	expected = 0x6ed0dd02806fa89e25de060c19d3ac86cabb87d6a0ddd05c333b84f4
	hash_digest = sha512_224(msg)
	print "Hash of '%s': %x\nMatches expected: %s." % (
