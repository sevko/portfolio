 ...
 * @return A pointer to an encrypted version of `plaintext` (also 8 bytes). If
 *      memory could not be allocated, return `NULL`.
 */
Byte_t *DES_encipher(const Byte_t *plaintext, const Byte_t *key);

/**
 * @brief Decrypt a block of ciphertext using DES.
 * @param plaintext An 8-byte block.
 * @param key An 8-block key.
 * @return A pointer to a decrypted version of `plaintext` (also 8 bytes). If
 *      memory could not be allocated, return `NULL`.
 */
