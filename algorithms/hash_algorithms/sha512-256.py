"""
`sha512-256` contains an implementation of the SHA-512/256 hash algorithm.
...
def sha512_256(msg):
	"""
	Generate the SHA-512-256 digest of a string.
	...
	bin_msg = "".join([bin(ord(byte))[2:].zfill(8) for byte in msg])
	bin_msg += "1" + (896 - 1 - len(bin_msg) % 1024) * "0"
	bin_msg += bin(len(msg) * 8)[2:].zfill(128)
